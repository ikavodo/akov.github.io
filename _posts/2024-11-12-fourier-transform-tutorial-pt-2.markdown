...
## Introduction
The Fourier convolution theorem[^1] is an extremely important result from the computational perspective, as with a sufficiently fast algorithm ([FFT](https://en.wikipedia.org/wiki/Fast_Fourier_transform)) the order of computational complexity ($O(N\log n$)) of a convolution operation becomes smaller than in a 'naive' implementation ($O(n^2)$). This means that implementing a convolution as a multiplication in the frequency domain is significantly faster than computing it in the time-domain (for large enough N). On the other hand, the modulation theorem (which is the dual of the previously mentioned one), defined as 
<div>
